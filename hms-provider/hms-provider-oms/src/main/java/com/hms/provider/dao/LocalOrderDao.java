...
public interface LocalOrderDao {
    ...
    @Insert({"insert into local_order_info (id, order_id, user_id, room_id, origin,check_in,check_out,check_in_status,count,hotel_id)",
            " values(#{id}, #{order_id}, #{user_id}, #{room_id}, #{origin}, #{check_in}, #{check_out}, #{check_in_status}, #{count}, #{hotel_id})",
            "ON DUPLICATE KEY UPDATE order_id = #{order_id}, user_id = #{user_id}, room_id = #{room_id},origin",
            " = #{origin},check_in = #{check_in},check_out = #{check_out},check_in_status = #{check_in_status},count = #{count}, hotel_id = #{hotel_id}"})
    ...
    int updateOrderStatus(@Param("status") int status,@Param("status") String orderid);
